...
Rx.LosElectricField = zeros(size(Rx.xyz,1),1);
% figure
TxRxRayList = [];
...
%     plot3(rayX,rayY,rayZ); %,'LineStyle','none','Marker','.','Color','Black');
    RxTxLosDist = (RxTx.dist(k,1,1))/1000;
    Rx.LosRssi(k)=Rx.LosRssi(k)+10.^((Tx.power(1)-(FPSLRefLoss+20.*log10(4*pi*...
        ((RxTxLosDist>=refDistance).*RxTxLosDist+(RxTxLosDist<refDistance)*refDistance).*...
        freq./lightVel)-10.*log10(prod(tempFresnelCoeff)))+...
       (TxAntennaGainAE(round(losBeamAngle.Tx.AziIndex(k,1)),round(losBeamAngle.Tx.ZenIndex(k,1))))+...
       (RxAntennaGainAE(round(losBeamAngle.Rx.AziIndex(k,1)),round(losBeamAngle.Rx.ZenIndex(k,1)))))...
       /10).*complex(cos(2*pi*freq*RxTxLosDist./lightVel+pi),sin(2*pi*freq*RxTxLosDist./lightVel + pi));

    Rx.LosElectricField(k)= Rx.LosElectricField(k)+...
        exp(-1i*2*pi*freq*RxTxLosDist./lightVel)./...
        ((RxTxLosDist>=refDistance).*RxTxLosDist + (RxTxLosDist<refDistance)*refDistance);
end
% hold off
