function [TxReflection2Rx,first2SecondPointj, first2SecondPointjDist, first2SecondIntd, reflectPoint2,reflecjRssiT, reflecjEFT]  = ReflectionRSSI(i,j,t,RxXYZ,Tx,wall,TxReflectXYZ2,antennaGainRes,Tx1XYZ,ceillFloor,TxReflectXYZList,FPSLRefLoss,lightVel,Rx2TxRefl,freq,TxAntennaGainAE,RxAntennaGainAE)
    ...
    for k = 1:size(TxReflectXYZ2,1)
        ...
        if (first2SecondIntd < 1 && first2SecondIntd > 0) 
            ...
            [reflecjRssi(k), reflecjEF(k)] = RssiFunc(t,Tx,FPSLRefLoss,first2SecondDist,1,Tx2FirstReflPintTransCoeff1,Tx2FirstReflPintTransCoeff2,Tx2FirstReflPintTransCoeff3,tempReflecCoeff(1,:),tempReflecCoeff(2,:),depBeamAngle,arrBeamAngle,lightVel,first2SecondDist,freq,TxAntennaGainAE,RxAntennaGainAE);
