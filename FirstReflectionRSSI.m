function [tempReflecCoeff,first2SecondPointj, first2SecondPointjDist,depBeamAngle, arrBeamAngle, first2SecondIntd, reflectPoint,reflecjRssi,reflecjEF]  = FirstReflectionRSSI(i,j,t,RxXYZ,wall,TxReflectXYZList,Tx1XYZ,antennaGainRes,ceillFloor,FPSLRefLoss,lightVel,Tx,Rx2TxRefl,freq,TxAntennaGainAE,RxAntennaGainAE,k)
...
if (first2SecondIntd < 1 && first2SecondIntd > 0) 
    ...
    if size(tempReflecCoeff,1) == 1
        [reflecjRssi, reflecjEF] = RssiFunc(t,Tx,FPSLRefLoss,first2SecondPointjDist(1,:),first2SecondPointjDist(2,:),Tx2FirstReflPintTransCoeff1,Tx2FirstReflPintTransCoeff2,1,tempReflecCoeff(1,:),1,depBeamAngle,arrBeamAngle,lightVel,Rx2TxRefl.dist(i,1,j,1),freq,TxAntennaGainAE,RxAntennaGainAE);
    else
        [reflecjRssi, reflecjEF] = RssiFunc(t,Tx,FPSLRefLoss,first2SecondPointjDist(1,:),first2SecondPointjDist(2,:),Tx2FirstReflPintTransCoeff1,Tx2FirstReflPintTransCoeff2,1,tempReflecCoeff(1,:),tempReflecCoeff(2,:),depBeamAngle,arrBeamAngle,lightVel,Rx2TxRefl.dist(i,1,j,1),freq,TxAntennaGainAE,RxAntennaGainAE);
    end
end
end 